...
17. [Dronterapia Szinkronizáció](#-dronterapia-szinkronizáció)
18. [Exportálás és Importálás](#-exportálás-és-importálás)
19. [Kamera Beállítások](#-kamera-beállítások)
20. [Élő Kamera Feed (CAM)](#-élő-kamera-feed-cam)
21. [Státuszsáv](#-státuszsáv)
22. [Drón Profilok](#-drón-profilok)
23. [Akkumulátorcsere – Nagy Területek](#-akkumulátorcsere--nagy-területek)
24. [Offline Térkép](#-offline-térkép)
25. [Képernyőkép és Videórögzítés (REC)](#-képernyőkép-és-videórögzítés-rec)
26. [Hibaelhárítás](#-hibaelhárítás)
27. [Tippek és Trükkök](#-tippek-és-trükkök)
...
✅ **Automatikus misszió generálás** – GSD alapú repülési magasság, kígyózó (lawnmower) és kettős rács (crosshatch) mintázat
...
✅ **Dronterapia szinkronizáció** – Repülési tervek feltöltése és letöltése a Dronterapia webes platformra; bejelentkezés felhasználónévvel és jelszóval
...
### Kettős rács mód (Crosshatch)

A **Crosshatch** kapcsolóval engedélyezhető a kettős rács mintázat: az app **két egymásra merőleges** sávsorozatot generál egyetlen misszióba.

```
Kígyózó (alapértelmezett):    Crosshatch:
═══════════════════           ═══════════════════
               ←              ═══════════════════
═══════════════════           ║   ║   ║   ║   ║
→                             ║   ║   ║   ║   ║
...
- 3D modellezéshez (jobb textura-lefedettség)
- Dombos vagy egyenetlen terepnél (oldalirányú átfedés biztosítása)
- Ha az egyirányú sávmintázatnál hiányos az átfedés a terület sarkainál

**Crosshatch szög (másodirány):**

A panel egy második szöget is megjelenít (alapértelmezetten az elsőre merőleges, azaz `irány + 90°`). Igény szerint módosítható.

> ⚠️ **Fontos:** Crosshatch módban a waypointok száma megduplázódik. Nagy területnél könnyen meghaladhatja a MSDK v4 korlátját (99 waypoint/misszió). Ebben az esetben az akkumulátorcsere-felosztás automatikusan aktiválódik.
...
A tervlistában **hosszan tartva** egy terven törlési mód aktiválódik:

1. Nyomd meg a **„Terv betöltése"** gombot
2. **Tartsd hosszan** a törölni kívánt tervet
3. Megerősítő kérdés jelenik meg: *„Törlöd a [név] tervet? A szerveren is törlődik."*
4. **„Törlés"** → a helyi fájl azonnal törlődik, és az app elküld egy törlési kérést a Dronterapia szerverre is
5. Toast üzenet jelzi az eredményt:
   - `Terv törölve (helyi + szerver)` – mindkét helyen törlés sikeres
   - `Helyi terv törölve (szerverről nem sikerült)` – helyben törlődött, de szerveres törlés sikertelen (pl. nincs internet)

> **Megjegyzés:** A törlés azonnali, nem kell utána szinkronizálni. Ha a szerveres törlés nem sikerült, a következő alkalommal manuálisan törölheted a webes felületen.
...
## 🔄 Dronterapia Szinkronizáció

A DroneFly képes kétirányúan szinkronizálni a repülési terveket a **Dronterapia webes platformmal** (`app.dronterapia.hu`). Így a tábléten elmentett tervek megjelennek a weben, és a weben létrehozott tervek letölthetők a tabletre.

### Bejelentkezés

A szinkronizáció előtt egyszer be kell jelentkezni a Dronterapia fiókba:

1. A jobb oldali panelen nyomd meg a **„Bejelentkezés"** gombot
2. A megjelenő dialógban add meg:
   - **Felhasználónév:** a Dronterapia fióknév (pl. `csabazsee`)
   - **Jelszó:** a Dronterapia belépési jelszó
3. Nyomd meg az **„OK"** gombot
4. Sikeres belépés esetén a gomb szövege **„Kijelentkezés (felhasználónév)"** lesz

> 💡 A token a tablet tárhelyén marad — következő indításkor nem kell újra bejelentkezni, amíg a session érvényes.

> ⚠️ **Lejárt token:** Ha a szerver visszautasítja a szinkronizálást (lejárt session), a státusz pirosra vált: *„Lejárt token – jelentkezz be újra"*. Ilyenkor ki kell jelentkezni és újra bejelentkezni.

### Szinkronizálás

Bejelentkezés után a **„Szinkronizálás"** gomb elérhető:

1. Nyomd meg a **„Szinkronizálás"** gombot
2. Az app:
   - **Feltölti** a tableten mentett, még nem szinkronizált terveket a Dronterapia szerverre
   - **Letölti** a Dronterapia webes felületen létrehozott terveket a tabletre
3. Szinkronizálás végén toast üzenet jelenik meg az eredményről

**Mit tölt fel automatikusan?**

| Feltétel | Eredmény |
|----------|----------|
| Tablet-en mentett, `sync_pending: true` jelzésű terv | Feltöltődik a szerverre |
| Tablet-en lévő régi terv, ami még nem szerepel a szerveren | Feltöltődik |
| Szerveren már létező terv módosítva lett | Felülírja a szerver verzióját |

**Mit tölt le?**

| Feltétel | Eredmény |
|----------|----------|
| Szerveren létező terv, ami nincs a tableten | Letöltődik |
| Szerveren frissebb verzió van | A tablet verzióját felülírja |

> ⚠️ **Offline használat:** Ha nincs internet-kapcsolat, a szinkronizáció nem indul el. A tervek helyben megmaradnak a tableten, és a következő szinkronizáláskor feltöltődnek.

### Szinkronizáció állapota

A státuszjelző mutatja az aktuális állapotot:

| Felirat | Szín | Jelentés |
|---------|------|----------|
| `Online – [felhasználónév]` | Zöld | Bejelentkezve, szinkronizáció lehetséges |
| `Online – nincs bejelentkezve` | Narancs | Internet van, de be kell jelentkezni |
| `Lejárt token – jelentkezz be újra` | Piros | Session lejárt – ki- és visszajelentkezés szükséges |
| `Kész: ↑N ↓M` | Zöld | Szinkronizálás kész (N feltöltött, M letöltött terv) |
| `Offline – nincs hálózat` | Szürke | Nincs internet-kapcsolat |
| `Sync letiltva repülés közben` | Narancs | Misszió fut – sync szünetel |

### Munkafolyamat (tablet ↔ web)

```
1. [Tablet] Repülési terv tervezése és mentése (SAVE)
         ↓
2. [Tablet] „Szinkronizálás" gomb megnyomása
         ↓
3. [Web] Dronterapia → ✈️ Flight Planning → Mentett tervek
         → A terv megjelenik a listában, exportálható KMZ / CSV formátumban
```

```
1. [Web] Dronterapia-ban repülési terv létrehozása és mentése
         ↓
2. [Tablet] „Szinkronizálás" gomb megnyomása
         ↓
3. [Tablet] Terv betöltése → azonnal generálható és feltölthető a drónra
```
