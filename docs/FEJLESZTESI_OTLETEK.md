...
**Frissítve:** 2026-07-02 (2) — a 4. lépés (Edge AI) teljes L1–L4 tervdokumentációt
kapott: [`M09_EDGE_AI_TOSZAMLALAS/`](M09_EDGE_AI_TOSZAMLALAS/). A médialetöltés
kérdése (fotók áttöltése a tabletre a mintavételi misszióból) **már megoldott** —
ez a 3. lépés része volt (`MediaSessionDownloader`, M04 §16), nem külön feladat.
...
- **DroneFly M09 (Edge AI Tőszámlálás):** a 4. lépés (tabletes YOLO-inferencia
  + azonnali db/ha és teljes területi extrapoláció) részletes terve — ld.
  [`M09_EDGE_AI_TOSZAMLALAS/`](M09_EDGE_AI_TOSZAMLALAS/)
...
4. 🔲 **Edge AI:** tabletes TFLite (YOLOv8n/YOLO11n INT8) inferencia + azonnali
   db/ha és teljes területi extrapoláció. Implementáció még nincs elkezdve, de a
   teljes architektúra-terv (L1–L4) kész: [`M09_EDGE_AI_TOSZAMLALAS/`](M09_EDGE_AI_TOSZAMLALAS/)
   (2026-07-02). Előfeltétel (M04 kis kiegészítés): a `session.json` bővítése
   `sample_altitude_m` + `drone_profile_name` mezőkkel, hogy a mintaponti kép
   földi lefedettsége (és így a sűrűség) automatikusan számolható legyen —
   ez küszöböli ki a Dronterapia POC-nál még szükséges kézi GSD-bevitelt.
5. 🟡 **Kriging / IDW interpoláció:** IDW megvan (Python, Dronterapia
   `utils/interpolation.py`), kriging még nincs. Az M09 v1 terve tudatosan **nem**
   tartalmaz térbeli interpolációt a tableten (csak átlagsűrűség × terület) —
   ld. `M09_L2_DONTESI_LOGIKA.md` §7 indoklás.
...
eszközön még nem lett kipróbálva. Az Edge AI (4. lépés) tervdokumentációja kész
(M09, 2026-07-02), implementáció még nem kezdődött el.
...
### Terepi teszt, 2026-07-03 — 3 hiba azonosítva, javítás implementálva (eszközön még nem tesztelve)

Az első valódi terepi próba (mintavételi misszió + szabályos grid-misszió,
Crystal Sky + P4P v1) három egymástól független, de az elsőt kivéve
összefüggő hibát tárt fel. Mindhárom **dokumentálva** (L1–L4 szinten) **és
implementálva** (`./gradlew compileDebugJavaWithJavac` sikeres, 2026-07-03),
de valódi Crystal Sky + Phantom 4 Pro v1 eszközön még egyik javítás sincs
kipróbálva — ez a következő lépés.

1. **Mintavételi fotó-trigger megbízhatatlanság:** 10 mintapontos misszióból
   csak 6 fotó készült (mind a 10 leszállás megtörtént). Ok: a
   `START_TAKE_PHOTO` WaypointAction "tűzz és felejtsd el" jellegű, nincs
   SDK-szintű visszaigazolás/újrapróbálkozás. Javítás: app-vezérelt
   `Camera.startShootPhoto()` hívás (`CameraConfigurator.triggerSamplePhoto()`)
   `onWaypointReached()`-ből, 1 újrapróbálkozással; a médialetöltés a
   ténylegesen megerősített pontok listájával dolgozik a nyers
   mintapontszám helyett. Részletek: `M04_L1` §18, `M04_L2` §10, `M04_L3`,
   `M04_L4` §10.

2. **Fotószám-becslés + valós átfedés-hiány (súlyosabb, mint gondoltuk):**
   a szabályos grid-misszió becslése (UI: "becsült képszám") akár 4×-esen
   túlbecsülhet, mert távolság-alapú képletet használ, miközben a kamera
   ténylegesen **időalapú**, 2 mp-es hardveres alsó korláttal triggerel
   (`CameraConfigurator.java:235`). Ez nem csak a UI-számot torzítja: **a
   ténylegesen repült átfedés is a beállított alá esik**, alacsony (< ~20 m,
   80% frontlap mellett — a küszöb frontlaptól függ) magasságon ez
   **ortomozaik-hézagokhoz** vezethet — egy napraforgó tábláról 10 m
   magasságon, 75/80% átfedéssel készült felvételekből a mozaik a terület
   <10%-áról állt csak össze, míg egy korábbi 18 m-es felmérés sikeres volt
   (ez utóbbi éppen a hiba küszöbértéke közelében van). Javítás: az
   effektív fotótávolság (`max(photoDist, speedMs×2)`) alapján számol a
   becslés, és UI figyelmeztetés jelzi, ha a kamera 2 mp-es korlátja aktív.
   Részletek + a veszteség képlete és küszöbtáblázata: `M02_L1` §8
   (különösen §8.3), `M02_L2` §10.
3. **Mintavételi beállítások és kamera-formátum (JPEG/RAW) nem mentődik
   projekt-mentéskor:** a `ProjectManager` JSON-sémája nem tartalmazta a
   `MissionConfig` mintavételi mezőit sem a `CameraSettings`-et. Javítás:
   additív `sampling` + `camera_settings` JSON blokk — a meglévő
   `opt*()`-alapú betöltés miatt **nincs szükség migrációra**, a régi tervek
   változatlanul betölthetők maradnak. Részletek: `M03_L1` §7, `M03_L3`.

**Tanulság a 2. ponthoz:** a hiba adott konfigurációnál (alacsony magasság +
nagy átfedés — pont a tőszámláláshoz szükséges beállítások) **rendszeresen**
jelentkezik, nem eseti — a terepi validáció előtt érdemes minden korábbi,
hasonlóan alacsony magasságon készült felmérést átnézni a `M02_L1` §8.3
küszöbtáblázata alapján, mielőtt ortomozaik-készítésre időt fordítanánk rá.

**Fontos kiegészítés (2026-07-03, ugyanezen a napon):** kiderült, hogy a 2.
pont javítása (helyes becslés + figyelmeztetés) **nem oldja meg** a mögöttes
problémát teljes ortomozaikhoz — a CURVED+intervallum mód **strukturálisan**
képtelen 20 m alatt (80% frontlapnál) elérni a beállított átfedést, mert a
kamera 2 mp-es fizikai korlátja és az MSDK 2 m/s-es minimum sebessége
együttesen egy áthidalhatatlan alsó korlátot ad. Az egyetlen valódi megoldás
(a mintavételi misszió megbízható, app-vezérelt fotó-triggerének a teljes
rácsra általánosítása, NORMAL móddal) megtervezve **és implementálva**:
**M10 — Sűrű Rács** ([`M10_SURU_ALACSONY_RACS/`](M10_SURU_ALACSONY_RACS/)).
A felhasználó 2026-07-03-án előbb csak a terv dokumentálását kérte, majd
tisztázta a szükségletet ("igazán nagyfelbontású ortomozaik, kísérleti
jelleggel" — ahol a mintavételes M09 statisztikai megközelítés nem elég) és
jóváhagyta a megvalósítást. Új UI-kapcsoló ("🐢 Sűrű rács") a
misszió-tervező panelen; kölcsönösen kizárja a mintavételi módot.
`./gradlew assembleDebug` sikeres, eszközön még nem tesztelve.
